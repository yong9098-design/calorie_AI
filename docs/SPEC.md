...
CALO AI는 음식 사진 한 장으로 칼로리와 영양소를 AI가 자동 분석하고, 사용자가 직접 수정·저장·추적할 수 있는 모바일 최적화 칼로리 관리 웹 앱이다. Gemini API(Flash/Pro)로 이미지를 분석하고, Supabase로 인증 및 데이터를 관리한다. 온보딩에서 Harris-Benedict 공식으로 TDEE를 자동 계산하여 개인화된 목표를 제공한다.
...
**데이터 흐름**:
- [완료] 클릭 → `saveOnboarding()` 호출
- `localStorage.setItem('onboarding_temp', JSON.stringify(data))` + `onboarding_complete = 'true'` 저장
- **분기 처리** (`currentUser` 기준):
  - 로그인 회원: `syncOnboarding(currentUser.id)` → Supabase profiles UPSERT → `onboarding_temp` 삭제 → 홈 화면(`showMain('home')`)
  - 게스트 모드: `applyGuestProfile(data)` → `onboarding_temp` 삭제 → 홈 화면
  - 미인증: 기존 로그인 화면(`showScreen('auth')`)으로 이동
...
- **스크롤 컨텐츠**:
  ...
  2. 매크로 진행률 섹션
  3. 오늘 식단 리스트 섹션
- **FAB (Floating Action Button)**: 오른쪽 하단, 카메라 SVG 아이콘(60px, 초록 그라디언트) → 바텀시트 열기
...
- 빈 상태: "📸 카메라 버튼을 눌러 음식을 찍어보세요" (회색 일러스트)
...
openBottomSheet()        // 카메라/갤러리 선택 바텀시트 열기 (FAB 클릭)
closeBottomSheet()       // 바텀시트 닫기
openCamera()             // capture="environment" input 트리거 (스마트폰 카메라)
openGallery()            // 일반 file input 트리거 (갤러리 피커)
handleImageFile(file)    // 공통 이미지 처리 → Gemini 분석 시작
```

#### [필수] 카메라/갤러리 Input HTML 명세

두 개의 `<input type="file">` 요소를 반드시 분리하여 구현. `capture` 속성은 Generator가 재생성할 때도 절대 제거되면 안 됨.

```html
<!-- 카메라 촬영용 input — capture="environment" 속성 필수 -->
<input type="file" id="img-input-camera" accept="image/*" capture="environment" style="display:none">

<!-- 갤러리 선택용 input — capture 속성 없음 (의도된 동작) -->
<input type="file" id="img-input-gallery" accept="image/*" style="display:none">
```

⚠️ **주의사항**:
- `capture="environment"` 없으면 모바일 카메라 대신 기본 파일 선택기(갤러리)가 열림
- 두 input을 하나로 합치거나, JavaScript에서 동적으로 `capture` 속성을 추가하는 방식은 사용 금지
- 데스크톱에서는 `capture` 속성이 무시되어 파일 선택기가 열리는 것이 정상 동작
...
**Gemini API 프롬프트** (`api/analyze.js` `ANALYSIS_PROMPT`):
```
...
```
- `responseMimeType: 'application/json'` + `responseJsonSchema` 구조화 응답으로 JSON 파싱 안정성 보장
- `components` 배열: 개별 음식 항목별 `name`, `quantity`, `calories_min`, `calories_max`, `note` 포함
...
#### 레이아웃
- **헤더**: 타이틀 + 액션 버튼 2개 (🗃 음식DB, + 추가)
  - "기록" 타이틀 (좌측)
  - [🗃 음식DB] 버튼 (우측, 초록)
  - [+ 추가] 버튼 (우측, 초록)
- **날짜 네비게이터**: 좌우 화살표로 날짜 이동 + 날짜 텍스트 클릭 시 캘린더 피커 (선택)
...
### 화면 7-B: 음식 데이터베이스 모달 (Food DB Modal)

#### 역할
기록 탭에서 [🗃 음식DB] 버튼 클릭 시 나타나는 모달. 사전 정의된 음식 목록(최근 음식, 인기 음식) 또는 한국 정부 식품영양DB API에서 검색한 음식들을 다중 선택하여 한 번에 여러 음식을 기록한다. (FR-12)

#### 레이아웃
- **모달 스타일**: Bottom Sheet (화면 중앙~하단, 배경 dimmer)
- **헤더**: "🗃 음식 데이터베이스" + 닫기 버튼
- **검색바**: 텍스트 입력 + 검색 아이콘
- **컨텐츠 영역** (3구간):
  1. **최근 음식** 섹션 (⏱ 최근 음식)
  2. **인기 음식** 섹션 (⭐ 인기 음식)
  3. **API 검색 결과** 섹션 (동적 표시)
- **푸터**: 선택 음식 카운터 + 끼니 타입 선택 드롭다운 + [저장] [취소] 버튼

#### UI 컴포넌트

**검색바**:
- 텍스트 입력: placeholder "음식 검색..."
- 실시간 필터링: 최근 음식 + 인기 음식 필터
- 300ms 디바운스 후 `/api/food-search` API 호출 (한국 정부 식품영양DB)

**음식 카드** (최근/인기/검색 결과):
- 체크박스 (좌측, 선택 시 카드 배경 초록)
- 음식명 (굵게, 14px)
- 사이즈 선택 버튼 (작음/중간/큼, toggle 스타일)
- 영양소 정보 (4줄):
  - 🔥 칼로리 (빨강)
  - 💪 단백질 (파랑)
  - 🌾 탄수화물 (주황)
...
**푸터**:
- 선택 음식 카운터: "선택된 음식: X개" (초록)
- 끼니 타입 선택: `<select>` (🍳 아침 / 🍱 점심 / 🍽️ 저녁 / 🍪 간식)
- [저장] 버튼 (초록, 선택 0개일 때 비활성화)
- [취소] 버튼 (회색 outline)
...
fooddbData = {
  recent: [
    { id, name, qty:'small|medium|large',
      cals:{small, medium, large},
      protein:{small, medium, large},
      carb:{small, medium, large},
      fat:{small, medium, large} }
  ],
  popular: [ /* 동일 구조 */ ]
}
```

**API 검색 결과** (한국 정부 식품영양DB):
```json
[
  { id, name, qty, cals, protein, carb, fat }
]
```
...
1. **모달 열기**: `openFoodDBModal()` → `renderFoodsDB()` (최근/인기 음식 표시)
2. **검색**: 사용자 입력 → `filterFoodsDB()` → 300ms 디바운스 → `searchFoodsFromApi(q)` → `/api/food-search?q=...&rows=20` → `fooddbApiResults` 갱신 → `renderFoodsDB()`
3. **선택**: 체크박스 클릭 → `toggleFoodDB(foodId, name)` → `fooddbSelected` 갱신 → `updateFooddbUI()`
4. **사이즈 변경**: `changeQtyDB(foodId, qty)` → 해당 음식의 qty 업데이트 → 카드 재렌더링
5. **저장**: 끼니 선택 후 [저장] 클릭 → `saveFoodsDB()` → 선택 음식별 `meal_logs` INSERT → `meal_logs` 테이블에 행 추가 → 모달 닫기 → 기록 탭 갱신
6. **닫기**: [취소] 클릭 → `closeFoodDBModal()` → 선택 초기화

#### 핵심 JS 함수
```js
openFoodDBModal()                    // 모달 표시 + 상태 초기화
closeFoodDBModal()                   // 모달 숨김 + 선택 초기화
renderFoodsDB()                      // 최근/인기/검색 섹션 렌더링
renderFooddbSection(id, foods)       // 특정 섹션 렌더링 (카드 생성)
toggleFoodDB(foodId, name)           // 음식 선택/해제
changeQtyDB(foodId, qty)             // 사이즈 변경
filterFoodsDB()                      // 검색 입력 → 디바운스 → API 호출
searchFoodsFromApi(q)                // `/api/food-search` 호출
updateFooddbUI()                     // 선택 카운터 + 버튼 상태 갱신
saveFoodsDB()                        // 선택 음식 → meal_logs 일괄 INSERT
saveMealDirect(meal)                 // 단일 meal 저장 (Supabase 또는 localStorage)
```
...
**요청**:
```
GET /api/food-search?q={search_term}&rows=20
```
...
#### 상태 관리
```js
let fooddbSelected = {};           // 선택 음식 {foodId: {name, qty}}
let fooddbIsLoading = false;       // 검색 중 여부
let fooddbSearchTimer = null;      // 디바운스 타이머
let fooddbApiResults = [];         // API 검색 결과
```
...
**섹션 2: AI 모델 선택**
- 라디오 그룹:
  - Gemini Flash (기본) — 빠른 응답, 일반 정확도 / 뱃지: "하루 20장 무료" (`.model-badge`, 초록)
  - Gemini Pro — 높은 정확도, 느린 응답 / 뱃지: "장당 3원 · Gemini 2.5" (`.model-badge`, 보라)
...
**섹션 5: 계정 관리**
- [로그아웃] 버튼 (빨간 텍스트 + 아이콘) → 커스텀 확인 모달(`#logout-modal`) 표시 후 실행. 회원/게스트에 따라 모달 문구 동적 변경
...
saveGoals()                // profiles 테이블 UPDATE (목표 칼로리/매크로) + 저장 토스트 표시
updateGeminiModel(model)   // 모델 선택 저장
openLogoutModal()          // 로그아웃 확인 모달 열기 (회원/게스트 분기)
executeLogout()            // 로그아웃 실행 + 인증 화면 이동
openResetModal()           // 재설정 확인 모달 열기
executeReset()             // 온보딩 데이터 초기화 + 온보딩 화면으로 이동
openGuestModal()           // 비회원 닉네임 입력 커스텀 모달 열기
confirmGuestModal()        // 닉네임 확정 + 게스트 세션 시작
showToast(msg)             // 하단 토스트 알림 (2.2초 자동 소멸)
```
...
> **아키텍처 변경 (2026-04-23)**: `src/` 소스 폴더와 `build.js` 빌드 스크립트 삭제. `output/index.html`을 직접 편집하는 단일 파일 방식으로 완전 전환. 로컬 개발 서버는 `local-server.js` (구 `server.js`)로 실행.

> **배포 (2026-04-23)**: GitHub `https://github.com/yong9098-design/calorie_AI` + Vercel 프로젝트 `calorie-ai-gamma.vercel.app`. `vercel.json`에 `builds` 배열 명시 필수 — 없으면 Vercel이 루트를 Node 서버로 오인하여 `/api/*` 서버리스 함수 미인식.
